...
class KeyFinder(AudioAnalyzer):
    ...
    def analyze(self, data):
        ...
        harmonics = librosa.effects.harmonic(data)
        tf = TonalFragment(harmonics, sr=self.sample_rate)
        self._processing = False
        return tf.key
...
def find_key(key_finder=KeyFinder()):
    def _find_key(source):
        def subscribe(observer, scheduler=None):
            def on_next(data):
                try:
                    key = key_finder.analyze(data)
